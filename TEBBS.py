...
def read_t_denom_file(gver):
    ...
    fcur = open('/Users/lahayes/NPP/qpp_stats_study/TEBBS/temperature_denom_g'+str_gver+'.txt')
...
def read_ratio_t_file(gver):
    ...
    if (len(str_gver) < 2): str_gver = '0'+str_gver
    # opening temp-denom file
    fcur = open('/Users/lahayes/NPP/qpp_stats_study/TEBBS/temperature_ratio_g'+str_gver+'.txt')
    # finding number of strings in the file
    ratio_t = []
    for line in fcur:
...
def find_goesfile(time):
    ...
    filename_short = '/Users/lahayes/NPP/qpp_stats_study/TEBBS/goes_rawdata/go'+gver+file_postfix+'.fits'
...
def TEBBS_calculate(start_time, end_time, plot_key = 0, sys_win = 0, savitzky_golay = 0, extend_50 = 0):
    ...
    if (mid_cross == False):
        ...
        if (os.path.isfile(goesfile_short) == False):
            ...
            shutil.move(ftemp, '/Users/lahayes/NPP/qpp_stats_study/TEBBS/goes_rawdata/')
